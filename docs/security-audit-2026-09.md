...
- **W-2 stays "mechanism ~75%"** — it genuinely depends on the reader copy-pasting attacker HTML
  into the composer; reply/forward *quoting* is already sanitised via `quotableHtml`
...
- **Fix:** render the Cc/Bcc rows when *either* list is non-empty
...
- **W-2 · Pasting from the reader into the composer bypasses `quotableHtml`** *(mechanism ~75%)* —
  ...
  again. *Fix:* add an `onPaste` handler that runs `text/html` through `quotableHtml` (fallback to
...
- **W-3 · The print document lets the mail's CSS hide or overlay the app-rendered header** —
  ...
  Paper/PDF integrity only; the live UI is unaffected. *Fix:* render the body in a nested
...
- **W-4 · The trusted-sender image gate keys only on the From address, even for quarantined mail** —
  ...
  and the client never sees it. *Fix:* do not apply the trusted-sender (or "always") bypass for mail
...
- **W-5 · The Unsubscribe dialog omits the mailto target on One-Click, but still sends it**
  *(regression of server F-3 / commit 2e98b2c)* — `src/features/mail/Unsubscribe.tsx:44`. For
  ...
  before anything is sent. *Fix:* derive one plan from the headers in one place
...
- **W-6 · Attachment names whose only dot is the first character skip the dangerous-file warning** —
  ...
  (`uwumail-core`) flags the same names. *Fix:* match the engine — `dot < 0 ? "" :
...
- **W-7 · The dangerous-extension list omits `svg`, `rdp`, `wsb`, `pub`, `desktop`** —
  ...
  silently. The preview itself is safe (`<img>`); only the saved file matters. *Fix:* add these to
...
- **W-8 · The misleading-link warning is bypassed by invisible format characters in the link text** —
  ...
  the impact is losing the warning. *Fix:* strip Unicode format characters
...
- **W-9 · `misleadingLink` ignores `mailto` header fields and fails open on an unparseable target** —
  ...
  the warning entirely. *Fix:* collect every recipient host — the comma-separated path plus the `to`,
...
- **W-10 · Reopening a server draft silently drops its Bcc recipients (and reply threading)** —
  ...
  W-1), reported because it concerns Bcc handling. *Fix:* request `bcc` in `openDraft`'s `Email/get`
...
- **W-11 · The PDF preview frame has no `sandbox` (unreachable on JMAP today)** —
  ...
  not turned into a live path when the (functional) preview is fixed. *Fix:* when making the preview
...
| F-8 · a click on an image map went around the link check | Every click goes through `requestOpenLink`/`LinkWarning` | **holds** — image-map clicks are caught like any other link (commit f4fe86c); W-8/W-9 are weaknesses *inside* `LinkWarning`'s heuristic, not a way around it. |
